...
//    }
    ...
    public void radixSort() {
        //exhaustive testing for valid output
        Random r = new Random();
        //test with big data
        int[] data = new int[r.nextInt(10000000)];
        for (int i = 0; i < data.length; i++) {
            data[i] = r.nextInt(i);
        }
        MathUtils.radixSort(data);
        Assert.assertTrue(isSorted(data));
        //smaller big data
        for (int test = 0; test < 50; test++) {
            data = new int[r.nextInt(100000)];
            for (int i = 0; i < data.length; i++) {
                data[i] = r.nextInt(i);
            }
            MathUtils.radixSort(data);
            Assert.assertTrue(Arrays.toString(data), isSorted(data));
        }
        //normal data
        for (int test = 0; test < 10000; test++) {
            data = new int[r.nextInt(1000)];
            for (int i = 0; i < data.length; i++) {
                data[i] = r.nextInt(i);
            }
            MathUtils.radixSort(data);
